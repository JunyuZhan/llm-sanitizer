...
## 2. Claude Code(Anthropic 协议,v0.2 支持)

```bash
# 网关上游配置为 Anthropic(控制台设置或环境变量):
#   LLM_SANITIZER_UPSTREAM=https://api.anthropic.com/v1
#   LLM_SANITIZER_KEY=sk-ant-...(控制台设置更安全,网关注入 x-api-key 头)

# Claude Code 指向网关:
export ANTHROPIC_BASE_URL="http://127.0.0.1:8790"
export ANTHROPIC_API_KEY="sk-ant-dummy"   # 占位即可,网关注入真实密钥
claude
```
