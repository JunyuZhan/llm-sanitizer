...
> PyPI 发布列入 v0.1 里程碑。当前请使用源码方式运行。
...
- **传输通道覆盖不全(R1)**:走 WebSocket 等非 HTTP 通道的客户端,v0.1 无法拦截。接入前请确认客户端使用 HTTP/SSE 传输。
- **识别覆盖有限(R2)**:正则规则无法覆盖所有姓名、别名、机构简称。高敏感场景需要自定义词表(v0.2)或本地模型。
- **脱敏 ≠ 匿名化(R3)**:占位符可防止明文外泄,但结合上下文("被告,男,30 岁,深圳")仍可能被推断。
- **还原只做精确回填(R4)**:模型改写占位符后无法还原。
