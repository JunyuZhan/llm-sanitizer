...
# 前邻仅排除字母数字(不排除汉字)——"车牌粤B12345""号牌京A88888" 是最高频写法,
# 若排除汉字前邻会漏报;省简称+大写字母+数字结构唯一,误报风险极低。
_PLATE = re.compile(
    r"(?<![0-9A-Za-z])[" + PROV_SHORT + r"][A-Z][A-HJ-NP-Z0-9]{5,6}(?![0-9A-Za-z])"
)
# 案号:(年份)+ 机关简称 + 法院代号 + 程序词 + 序号 + 号(全角/半角括号均可)
_CASE_NO = re.compile(
